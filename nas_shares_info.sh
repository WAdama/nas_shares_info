...
TOTAL=$((TOTAL + USED))
echo "<result><channel>Volume $SHARE: In Use</channel><value>$((USED*1024))</value><unit>BytesDisk</unit><VolumeSize>GigaByte</VolumeSize></result>"
if [ -d $REC ]
  then
	IFS=" " read RECY RECNAME <<< `du -s $REC`
	echo "<result><channel>Volume $SHARE: Recyclable</channel><value>$((RECY*1024))</value><unit>BytesDisk</unit><VolumeSize>GigaByte</VolumeSize></result>"
fi
done
